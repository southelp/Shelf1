...
serve(async (req) => {
  try {
    ...
    if (!book) {
      return new Response(JSON.stringify({ message: '도서를 찾을 수 없습니다' }), { status: 404 });
    }

    const { data: active } = await client.from('loans').select('id').eq('book_id', book_id).in('status', ['reserved', 'loaned']);
    if (active && active.length > 0) {
      return new Response(JSON.stringify({ message: '이미 예약/대여 중입니다' }), { status: 400 });
    }
    ...
    // ✨ 이메일 발송 부분을 별도의 try-catch로 감싸서 오류를 명확히 합니다.
    try {
      ...
      // 이메일 발송 실패 시 더 구체적인 오류 메시지를 반환합니다.
      return new Response(JSON.stringify({ message: '대출 요청은 생성되었으나 승인 이메일 발송에 실패했습니다. Function의 환경 변수(예: RESEND_API_KEY) 설정을 확인하세요.' }), { status: 500 });
    }

    return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json' } });
  } catch (e) {
    console.error('Overall function error:', e);
    return new Response(JSON.stringify({ message: '서버 오류: ' + e.message }), { status: 500 });
  }
});